...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from aojp Github (http://github.com/aojp)." % device)
...
page = 1
while not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/users/aojp/repos?per_page=200&page=%d" % page)
    add_auth(githubreq)
    result = json.loads(urllib.request.urlopen(githubreq).read().decode('utf8'))
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/aojp.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    f = open('.repo/local_manifests/aojp.xml', 'w')
    f.write(raw_xml)
    f.close()
...
print("Repository for %s not found in the aojp Github repository list. If this is in error, you may need to manually add it to your local_manifests/aojp.xml." % device)
sys.exit(1)
