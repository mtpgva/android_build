...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from mtpgva Github (http://github.com/mtpgva)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:mtpgva+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/mtp.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    f = open('.repo/local_manifests/mtp.xml', 'w')
    f.write(raw_xml)
    f.close()
...
print("Repository for %s not found in the mtpgva Github repository list. If this is in error, you may need to manually add it to your local_manifests/mtp.xml." % device)
sys.exit(1)
